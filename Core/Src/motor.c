...
void send_motor_speed(uint8_t id) {
  ...
  data[3] = (motor[id - 1] > 0) ? (uint8_t)(motor[id - 1] >> 8)
                                : (uint8_t)(-motor[id - 1] >> 8);
  data[4] =
      (motor[id - 1] > 0) ? (uint8_t)motor[id - 1] : (uint8_t)(-motor[id - 1]);
  data[5] = 0x00;
  data[6] = 0x01;
  data[7] = 0x6B;
...
bool pid_to_v(float X_target, float Y_target,
  ...
  if (X_OLD == 0.0) {
    ...
    vx = (double)(kp * (X_target - OPS_X) + kd * (X_OLD - OPS_X) +
                  ki * (xki_sum));
    vy = (double)(kp * (Y_target - OPS_Y) + kd * (Y_OLD - OPS_Y) +
                  ki * (yki_sum));
    if ((int)angle_taget == (int)current_r)
      wv = (double)((kp + 9.0) * (angle_taget - ops_cache) +
                    (kd + 0.3) * (angle_OLD - ops_cache) +
                    (ki + 0.0064) * (ang_sum)); // 边走边转
    else
      wv = (double)((kp + 2.0) * (angle_taget - ops_cache) +
                    (kd + 0.3) * (angle_OLD - ops_cache) +
...
bool pid_to_goal_relative(float X_target, float Y_target,
                          float angle_taget) { // 相对坐标
...
Astar路径规划 非阻塞版
        ...
        case NB_IDLE:
                ...
                if(path_length <= 2){// 与阻塞版一致：path_length<=2
时循环不执行直接返回 true nb_state = NB_DONE;
//（注意：起点终点相邻时不会移动，属阻塞版原有行为） return true;
                }
                nb_j = 2;
                nb_old = 0;         // 注意：阻塞版 old 是 static
跨任务不清零，这里每次新任务归零，
                                    // 如需与阻塞版完全一致可删除本行
        ...
        case NB_TURN:           // 原地转向：等效原
pid_to_goal(current_x,current_y,angle) if(pid_to_v(current_x, current_y,
nb_angle)){ nb_old = nb_angle; if(nb_j+2 < path_length && (abs(path[nb_j+2].y -
path[nb_j-2].y) == 4 || abs(path[nb_j+2].x - path[nb_j-2].x) == 4)){ nb_j += 2;
// 直行段跳过一个节点（与阻塞版一致）
                        }
                        nb_state = NB_MOVE;
                }
        ...
        case NB_MOVE:           // 走向当前节点
                if(pid_to_v(nodes[path[nb_j].x][path[nb_j].y].real_x ,
nodes[path[nb_j].x][path[nb_j].y].real_y , nb_angle)){ nb_j += 2; if(nb_j >=
path_length){// 全部走完 nb_state = NB_DONE; return true;
                        }
                        nb_angle = -90*(path[nb_j].x - path[nb_j-2].x)/2;
                        if(abs((int)(nb_angle - nb_old)) == 180)
                                nb_angle = nb_old;
...
/*
                case STATE_NAV:{
                        switch(posit_state){
                        case 0: if(pid_to_goal_nb(200, 200, 0)){ posit_state =
   1; } break; case 1: if(pid_to_path_nb(0,0,2,0)){    posit_state = 2; }
   break;//路径规划 case 2: if(pid_to_path_nb(2,0,4,2)){    posit_state = 3; }
   break; case 3: if(pid_to_path_nb(4,2,0,2)){    posit_state = 4; } break; case
   4: if(pid_to_path_nb(0,2,2,4)){    posit_state = 5; } break; case 5:
   if(pid_to_path_nb(2,4,4,2)){    posit_state = 6; } break; case 6:
   if(pid_to_path_nb(4,2,0,2)){    posit_state = 7; } break; case 7:
   if(pid_to_path_nb(0,2,2,4)){    posit_state = 8; } break; case 8:
   if(pid_to_path_nb(2,4,0,0)){    posit_state = 9; } break; case 9:
   if(pid_to_goal_nb(15, -15, 0)){//最后一步定位单独占一个状态 posit_state = 0;
