...
void Lub_Cat_receive_start() {
  HAL_UARTEx_ReceiveToIdle_DMA(&huart4, CAT_redata, CAT_data_len);
}
...
例：物料-红色 = AF FA 08 02 01 03 CF FC
    LEN = 0x08（7 + 1 字节数据），CMD = 0x02，DATA = 0x01，CS = 0x02 ^ 0x01 =
0x03

huart4 只配了接收 DMA（DMA2_Stream2），没有发送 DMA，所以发送用阻塞方式；
...
void Lub_Cat_send(uint8_t cmd, const uint8_t *data, uint8_t data_len) {
  ...
  HAL_UART_Transmit(&huart4, frame, CAT_FRAME_FIX_LEN + data_len, 100);
}
...
bool pid_to_cat(float angle_taget) {
  ...
  if (error_x > -ErrTol_cat && error_x < ErrTol_cat && error_y > -ErrTol_cat &&
